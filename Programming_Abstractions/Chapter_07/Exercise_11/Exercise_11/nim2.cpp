...
void display_game(state_t state) {
	...
	if (FIRST_PLAYER == Human) {
		cout << "You have " << state.first_player_coins << " coins in your pile." << endl;
		cout << "I have " << N_COINS - state.coins_left - state.first_player_coins << " coins in my pile." << endl;
	}
	else {
		cout << "You have " << N_COINS - state.coins_left - state.first_player_coins << " coins in your pile." << endl;
		cout << "I have " << state.first_player_coins << " coins in my pile." << endl;
	}
}
...
void make_move(state_t &state, move_t move) {
	if (state.whose_turn == FIRST_PLAYER)
		state.first_player_coins += move;
...
bool check_for_win(state_t state, player_t player) {
	if (player == FIRST_PLAYER)
		return (state.first_player_coins % 2 == 0);
	else
		return (state.first_player_coins % 2 == 1);
}
