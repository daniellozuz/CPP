...
void execute_command_once(EditorBuffer &buffer, string line) {
	string previous_string, replacement_string;
	switch (toupper(line[0])) {
	case 'I':
		...
		break;
	case 'S':
		buffer.search(line.substr(1, line.length() - 1));
		break;
	case 'R':
		previous_string = line.substr(1, line.find("/") - 1);
		replacement_string = line.substr(line.find("/") + 1);
		if (buffer.search(previous_string)) {
			for (int i = 0; i < previous_string.length(); i++)
				buffer.backspace_character();
			for (int i = 0; i < replacement_string.length(); i++)
				buffer.insert_character(replacement_string[i]);
		}
		break;
	case 'D':
		buffer.delete_character();
